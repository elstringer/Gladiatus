...
function hum_read_duration (seconds,totalSeconds,mode,showDiv,width) {
	...
	if (hours == 1) {
		if (mode == 0) {
			...
			ret_val = "<strong>This action is in 01:";
		}
	}
	else if (hours > 1) {
		if (mode == 0) {
			...
			ret_val = "<strong>This action is in " + hours + ":";
		}
		else {
			ret_val = "<strong>Deze actie is in 0" + hours + ":";
		}
	}
	else if (mode == 3) {
		ret_val = "<strong>Deze actie is in 00:";
	}
	if (minutes == 1) {
		if (mode == 0) {
			ret_val += "1 minute ";
	...
	if (seconds == 1) {
		...
		if (mode == 0) {
			...
			ret_val += seconds + " ended, so be quick!</strong>";
		}
		else {
			ret_val += "0" + seconds + " finished!</strong>";
		}
	}
	else if (mode == 3) {
		ret_val += "00";
